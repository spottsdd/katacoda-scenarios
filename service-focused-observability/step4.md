...
In our `/deploy/docker-compose/docker-compose-broken-instrumented.yml`{{open}} there's a command to bring up our Flask server. If we look at line 81, we'll see:
...
Automatic instrumentation is done via environment variables in our `/deploy/docker-compose/docker-compose-broken-instrumented.yml`{{open}} starting on line 73:
...
The last thing we need to add is a *label* to our container, so our logs are sent with the label of the service, and with the proper language pipeline processor on line 89:
